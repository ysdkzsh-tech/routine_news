...
5. 今週のトピックに関連する最新情報をWebSearch/WebFetchで収集する。**関連性の高い記事・情報源を最大3件に絞り**、それ以上は探索しない（実行時間を短く安定させるため）。出典は固定リストにせず、信頼性の高い一次情報・専門メディアを優先する（本ファイル末尾の「情報源の傾向」を参照）。連携先病院名は対外的に明示しない方針を厳守し、レポート内で実名を断定的に書かない。
