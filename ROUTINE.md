...
このファイルは、**毎週月曜7:00 JST**（cron式 `0 22 * * 0` UTC）に起動するクラウドエージェントが従う手順です。実行時刻はこのファイルが単一の正とする（他ファイルでは再掲しない）。`CONTEXT.md` と合わせて必ず読むこと。
...
8. `origin/main` にpushする：`git push origin main`
   - pushが失敗した場合（リモートが進んでいる等）は `git pull --rebase origin main` を1回実行し、再度pushする。それでも失敗する場合は、ローカルコミットは残したまま処理を終了し、失敗内容を最後の出力に明記する。
9. push成功後、最後の出力として「今週のトピック名・レポートファイル名・コミットハッシュ・情報収集サマリーの要点1〜2行」を出力する。
